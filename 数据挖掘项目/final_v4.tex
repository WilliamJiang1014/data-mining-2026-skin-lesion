...
\section{结果与分析}
本节报告患者级内部测试、跨数据集泛化和子群稳定性三个层面的实验结果。M0--M4 作为表格模型、图像模型和常规多模态融合模型的基线，M5 为本文提出的 \modelname{}。

其中，M0 用于衡量仅依赖临床元数据时的判别能力，M1--M3 用于比较不同视觉编码器在 3D-TBP 病灶裁剪图像上的表现，M4 用于检验图像与元数据在样本级融合后的增益，M5 则进一步引入患者上下文、属性节点、视觉近邻和知识原型，以评估显式图结构是否能够带来额外收益。除特别说明外，所有结论均基于五折患者级测试集的均值与标准差。患者级划分保证同一患者的病灶不会同时出现在训练集和测试集中，从而降低由患者身份或同次采集上下文带来的信息泄漏风险。模型选择、早停、超参数筛选和分类阈值确定均在验证集完成，测试集仅用于最终评估；因此，本节报告的测试性能反映的是模型在未见患者上的泛化能力，而不是对测试集阈值或模型配置进行后验调节后的结果。

结果分析按照由内到外的顺序展开。在内部患者级测试集上比较 M0--M5 的 pAUC、AUPRC、AUROC、Sensitivity 和 Specificity，重点观察 \modelname{} 是否在高灵敏度筛查区域取得更优风险排序。通过 PAD-UFES-20 外部验证分析模型从 ISIC 2024 SLICE-3D Permissive 迁移到智能手机临床图像域时的性能变化，以检验模型是否存在明显域偏移。最后，按性别、年龄段和解剖部位报告子群指标，用于评估模型在不同人群和病灶位置上的稳定性。
...
图\ref{fig:m5_fold_profile} 展示了 \modelname{} 在五折患者级测试协议下的折间稳定性。总体来看，AUROC 在五个测试折中均维持在较高水平，说明模型在不同患者划分下都能够保持较稳定的整体判别能力，即能够较好地区分恶性与良性病灶。然而，pAUC 的波动比 AUROC 更明显，这表明高灵敏度区间的排序性能对测试折中的阳性样本数量、阳性病灶难度以及阴性样本分布更加敏感。由于筛查任务更关注 TPR 较高区域内的特异度保持能力，因此 pAUC 的折间变化比 AUROC 更能反映模型在真实高召回应用场景中的不确定性。

从阈值相关指标看，FNR 在第 2 折最低，说明该折中模型在验证集确定的高灵敏度阈值下能够较好地控制漏诊；而第 4 折的 FNR 相对较高，提示不同患者子集之间可能存在病灶形态、解剖部位分布或阳性样本组成差异。换言之，即使模型在总体 AUROC 上保持稳定，固定阈值迁移到不同测试折时仍可能产生不同程度的漏检风险。与此同时，ECE 的变化说明不同测试折之间还存在校准差异，即模型输出的风险分数在不同患者分布下并不总是具有完全一致的概率解释。对于医学筛查场景而言，这一点尤其重要，因为风险分数不仅用于排序，也可能影响后续随访、复查或人工复核优先级。

因此，M5 的优势不能仅由某一个测试折或某一个单一指标解释，而应理解为在患者级五折协议下对高灵敏度排序能力、漏诊控制能力和校准稳定性的综合折中。具体而言，\modelname{} 在多数折中保持较高 AUROC，说明其基础判别能力较稳定；在 pAUC 上取得较好平均表现，说明图结构上下文有助于高灵敏度筛查区间的风险排序；但 FNR 和 ECE 的折间波动也提示，模型在阈值迁移和概率校准方面仍存在改进空间。后续工作可进一步结合温度缩放、分子群校准或 fold-specific calibration 分析，以降低不同患者分布下的阈值敏感性。
